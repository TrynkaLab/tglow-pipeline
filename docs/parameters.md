...
| `sc_autoscale_q1` | The measure_intensity feature stat to read per cell/image for dynamic-range scaling (e.g. 'q100' reads the ch<N>__q100 column). Only used to auto-derive a channel_map when sc_channel_map is null - ignored otherwise, since an explicit channel_map's rep_features_dynamic takes over. Valid options are whatever stat suffixes measure_intensity actually computes, e.g. 'min', 'q25', 'q50', 'mean', 'q75', 'q99', 'q99.9', 'q99.999', 'max'. | `string` | q100 |  |  |
| `sc_autoscale_q2` | Controls which quantile is taken across all images for chosen q1 Valid options: 0 - 1 | `number` | 0.95 |  |  |
